...
which dosemu &> /dev/null || which dosbox &> /dev/null || fatal "Dosbox not installed"
which dosemu &> /dev/null || [ -f "$CONF_FILE" ] || fatal "Dosbox configuration file does not exist: $CONF_FILE"
...
cat > $BAT_FILE <<EOF
echo on
set PATH=Z:\;\\dos\\$chain\\bin;\\dos\\$chain\binb;\\dos\\$chain
set INCLUDE=\\dos\\$chain\\include
set LIB=\\dos\\$chain\\lib
$cmdline > \\dos\\log.txt
EOF
...
dosemu -K "$(pwd)/" -E $BAT_FILE &> $emu_logfile
...
logfile=$(find dos/ -iname "log.txt")
